# This is the photon simulation
# input event list + root file
# output pkl
import numpy as np
...
def getLandau(dis,angle,timeLandau,PhotonCount):
    ...
    if angle < 45:
        ...
        eta = etas_on[np.searchsorted(etas_on[:,0], dis)-1,1]
        mu = mus_on[np.searchsorted(mus_on[:,0], dis)-1,1]
        k = k_on[np.searchsorted(k_on[:,0], dis)-1,1] 
    else:
        axis = 'off'
        eta = etas_off[np.searchsorted(etas_off[:,0], dis)-1,1]
        mu = mus_off[np.searchsorted(mus_off[:,0], dis)-1,1]
        k = k_off[np.searchsorted(k_off[:,0], dis)-1,1]
...
def Pix(values,idx):
    global TimeMapYZ
    global Edep_number, Edep_dq, Edep_x, Edep_z, Edep_y, Edep_t,Edep_length
    with uproot.open(HEAD_ROOT + G + '.root') as f:
        tree = f['event_tree']
        Edep_x = tree.array(['hit_start_x'])
        Edep_number = tree.array(['number_hits'])
        Edep_dq = tree.array(['hit_energy_deposit'])
...
def multi(aa):
    ...
    # PP = np.where(b == -13)[0].tolist()
    # muons = getParticles(idx,-13)
    # kaons = getParticles(idx,321)
    PP = np.arange(0,Edep_number[idx],1).tolist()        
    chunks = np.array_split(PP, cpu) 
    
    with Pool(processes=cpu) as pool:
        count = pool.map(partial(Pix,idx =idx), chunks)
        final = sum(count)
    return final
...
def start():
    ...
    with open(HEAD +'landau.pickle', 'rb') as f:
...
zLen = 23 # 23 cm by 24 cm (seconds) --> 4 x 4 cm (6-7 min) 
yLen = 24 #         
intTime = 2000             
cpu = 4 # number of cores used
start() 
...
# Read list of events from file Kdecay2Hadrons.txt
...
if __name__=='__main__':
    photons = []
    for dd in range(len(EventList)):
        start = timer()
        photons = multi(EventList[dd])
        ph = sum(sum(sum(photons)))
        # Creates the output photon count per event in the pkl
        f = open(DUMP + str(EventList[dd]) + '.pkl', 'wb')
        pickle.dump(photons, f)
        f.close()
        print(EventList[dd],ph)
        end = timer()
